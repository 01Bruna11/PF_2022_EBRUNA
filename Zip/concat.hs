--A função concatMap fn xs equivale a aplicar a função fn a cada elemento da lista xs e então concatenar os valores. Utilize a operação de fold e map para implementar concatMap.
--IN : Uma lista u, uma função fn
--OUT: Uma lista com a concatenação da aplicação da função map

concatMap :: (a -> [b]) -> [a] -> [b]
concatMap fn = foldr ( (++) . fn y) []
main = do
